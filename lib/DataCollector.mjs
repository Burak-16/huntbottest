...
function listAccount(data) {
    const obj = listCheckbox("list", "Giriş yapmak için bir hesap seçin /İtemSatış & FantasticShop Discord.gg/Uranyum", [
        ...new Set(Object.values(data).map(user => user.tag)), 
        {name: "/İtemSatış & FantasticShop Discord.gg/Uranyum Yeni Hesap Ekle (Token ile Giriş Yap)", value: 0},
        {name: "/İtemSatış & FantasticShop Discord.gg/Uranyum Yeni Hesap Ekle (QR Kod ile Giriş Yap)", value: 1},
        {name: "/İtemSatış & FantasticShop Discord.gg/Uranyum Yeni Hesap Ekle (Parola ile Giriş Yap - MFA Gerekli)", value: 2},
    ])
    obj.filter = (value) => {
...
function getToken() {
    return {
        ...
        message: "/İtemSatış & FantasticShop Discord.gg/Uranyum Tokeninizi Girin "
    };
}
...
function getAccount() {
    return [{
        type: "input",
        message: "/İtemSatış & FantasticShop Discord.gg/Uranyum Email/Telefon Numaranızı Girin: ",
        validate(ans) {
...
function listGuild(cache) {
    const obj = listCheckbox("list", "/İtemSatış & FantasticShop Discord.gg/Uranyum Botun çalışacağı sunucuyu seçin", client.guilds.cache.map(guild => ({name: guild.name, value: guild.id})))
...
function wayNotify(cache) {
    const obj = listCheckbox(
        "checkbox", 
        "/İtemSatış & FantasticShop Discord.gg/Uranyum Selfbot bir captcha aldığında nasıl bilgilendirilmek istediğinizi seçin", 
        [
            {name: "Müzik", value: 3},
...
function webhook(cache) {
    const obj = {
        type: "input",
        message: "/İtemSatış & FantasticShop Discord.gg/Uranyum Webhook bağlantınızı girin",
        validate(ans) {
...
function userNotify(cache){
    const obj = {
        type: "input",
        message: "/İtemSatış & FantasticShop Discord.gg/Uranyum Webhook/Arama/Direkt Mesaj ile bilgilendirilmek istediğiniz kullanıcı ID'sini girin",
        async validate(ans) {
...
function captchaAPI(cache) {
    const obj = {
        ...
        choices: [
            ...
            {name: "Nosia Captcha Çözme API [Çok Yakında]", disabled: true}
        ],
        loop: false
    }
    if(cache) obj.default = cache;
...
function botPrefix(cache) {
    const obj = {
        type: "input",
        message: "/İtemSatış & FantasticShop Discord.gg/Uranyum Entere Basınız.",
        validate(ans) {
            if(!ans) return true
...
function gemOrder(cache) {
    const obj = listCheckbox(
        "list", 
        "/İtemSatış & FantasticShop Discord.gg/Uranyum Hunt için gem kullanma sırası seçin", 
        [
...
export async function collectData(data) {
    ...
    try {
        ...
        log("Yeni token oluşturma başarısız oldu.", "e")
    }
