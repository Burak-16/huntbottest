...
export default {
    ...
    callback: async (message, ...args) => {
        try {
            ...
            if(!args) return message.reply("Cowoncy gönderilecek kişiyi etiketlemeli ve cowoncy miktarını girmelisiniz. /İtemSatış & FantasticShop Discord.gg/Uranyum")
            const target = message.mentions.members.first()
            const owo = message.guild.members.cache.get(global.owoID)
            if(!target) return message.reply("Cowoncy gönderilecek bir kişi etiketlemelisiniz. /İtemSatış & FantasticShop Discord.gg/Uranyum")
            if(!owo) return message.reply("OwO botu bulunamadı.")
            if(!args[1].match(/^[0-9]+$/)) return message.reply("Gönderilecek cowoncy miktarını girmelisiniz. /İtemSatış & FantasticShop Discord.gg/Uranyum")
